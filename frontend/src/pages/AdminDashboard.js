...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const getValue = (result, defaultValue) => 
        result.status === 'fulfilled' ? result.value.data.data : defaultValue;

      setStats(getValue(results[0], null));
      setLeads(getValue(results[1], []));
      setAppointments(getValue(results[2], []));
      setApplications(getValue(results[3], []));
      setUsers(getValue(results[4], []));
      setFeedback(getValue(results[5], []));
      setCibilChecks(getValue(results[6], []));
      setDocuments(getValue(results[7], []));

      // Check if any critical data failed to load
      const failedCount = results.filter(r => r.status === 'rejected').length;
      if (failedCount > 0) {
        toast.warn(`Failed to load some dashboard data (${failedCount} errors)`);
      }
    } catch (error) {
  ...
  const deleteItem = async (endpoint, id, label) => {
    ...
    try {
      await api.delete(`${endpoint.replace('/api', '')}/${id}`);
