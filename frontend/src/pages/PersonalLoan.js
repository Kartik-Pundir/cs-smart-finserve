...
const steps = [
  ...
  { n: '02', title: 'Check Your Eligibility',      desc: 'Instantly know your eligible loan amount and the best interest rate available to you.' },
  { n: '03', title: 'Upload Documents',            desc: 'Submit your ID proof and last 3 months salary slips — fully digital, no paperwork.' },
  { n: '04', title: 'Get a Personalised Offer',    desc: 'Receive a tailored loan offer with the best rate and repayment terms suited to your profile.' },
  { n: '05', title: 'Instant Approval',            desc: 'Get approval in as little as 2 hours from our lending partners.' },
  { n: '06', title: 'Dedicated Support',           desc: 'A relationship manager stays with you through every step until disbursement.' },
  { n: '07', title: 'Money in Your Account',       desc: 'Funds are credited directly to your bank account within 24 hours of approval.' },
];

const PersonalLoan = () => {
  const [formData, setFormData] = useState({
    fullName: '', email: '', phone: '', serviceType: 'Personal Loan',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ fullName: '', email: '', phone: '', serviceType: 'Personal Loan', loanAmount: '', employmentType: 'salaried', monthlyIncome: '', city: '' });
    } catch (err) {
  ...
  return (
    <div className="min-h-screen" style={{ background: '#faf8ff' }}>
      <section className="relative pt-40 pb-24 bg-cover bg-center"
        style={{ backgroundImage: 'linear-gradient(135deg, rgba(26,26,46,0.92) 0%, rgba(192,57,43,0.88) 100%), url(https://images.unsplash.com/photo-1579621970563-ebec7560ff3e?w=1400&q=80)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-white">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
            <span className="inline-block px-4 py-1 bg-white/20 rounded-full text-sm font-semibold mb-4 backdrop-blur-sm">Personal Loan</span>
            <h1 className="text-4xl md:text-6xl font-heading font-bold mb-4">Money When You Need It Most.</h1>
            <p className="text-white/80 text-lg max-w-xl mx-auto mb-8">No collateral. Minimal paperwork. Up to ₹25 Lakhs disbursed in 48 hours.</p>
      ...
      <section className="py-20" style={{ background: '#f0eeff' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            ...
            <h2 className="text-3xl font-heading font-bold text-gray-900">Why Choose Our Personal Loan?</h2>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {['Instant Approval in 2 Hours', 'No Collateral Required', 'Loan up to ₹25 Lakhs', 'Flexible Tenure 1–5 Years', 'Minimal Documentation', 'Competitive Interest Rates'].map((f, i) => (
      ...
      <section className="py-20" style={{ background: '#faf8ff' }}>
        <div ref={sectionRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            ...
            <h2 className="text-3xl font-heading font-bold text-gray-900">Applying for a Personal Loan was never this easy</h2>
          </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="bg-gradient-to-br from-accent to-red-800 rounded-3xl p-10 text-white shadow-2xl">
                ...
                <div className="space-y-3">
                  {['100% online process', 'No guarantor needed', 'Prepayment allowed'].map((p, i) => (
      ...
      <section id="apply" className="py-20" style={{ background: '#f0eeff' }}>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10">
            ...
            <h2 className="text-3xl font-heading font-bold text-gray-900">Apply for Personal Loan</h2>
          </motion.div>
